<img width="628" height="264" alt="изображение" src="https://github.com/user-attachments/assets/34367a30-c406-4cba-8889-63ed270fc2cb" />

# **A/B Testing for Cookie Cats**

Daset from Kaggle: https://www.kaggle.com/datasets/yufengsui/mobile-games-ab-testing

Cookie Cats - это мобильная головоломка, разработанная компанией Tactile Entertainment. Это классическая "три в ряд", где игрок должен соединять плитки одного цвета, чтобы очистить доску и выиграть уровень.

По мере прохождения уровней игроки сталкиваются с воротами, которые заставляют их либо подождать некоторое время, либо совершить покупку внутри приложения, чтобы продолжить. Помимо стимулирования внутриигровых покупок, такие ворота выполняют важную функцию: они принудительно дают игрокам перерыв в игре, что, как предполагается, повышает и продлевает удовольствие от игрового процесса.

Но где именно размещать эти ворота? Изначально первые ворота стояли на уровне 30. В этом проекте мы сделали A/B-тест, в котором первые ворота в игре Cookie Cats были перенесены с уровня 30 на уровень 40. В частности, мы рассмотрели влияние этого изменения на удержание игроков.

В результате перенос не улучшил метрики
